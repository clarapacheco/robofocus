...
function renderUpcomingTasks() {
    ...
    const allTasksDone = document.getElementById('all-tasks-done');
    
    if (!upcomingTasksList || !allTasksDone) return;
    ...
    if (incompleteTasks.length === 0) {
        ...
        upcomingTasksList.classList.add('hidden');
        allTasksDone.classList.remove('hidden');
        return;
    }
    
    // Show tasks list
    upcomingTasksList.classList.remove('hidden');
    ...
    tasksToShow.forEach(task => {
        ...
        html += `
            <div class="task-reminder-item" onclick="navigateTo('routine')">
                <div class="task-reminder-icon ${task.category}">
                    ${categoryIcons[task.category]}
                </div>
                <div class="task-reminder-details">
                    <div class="task-reminder-title">${task.title}</div>
                    <div class="task-reminder-time">${task.time}</div>
                </div>
                <div class="task-reminder-status" onclick="event.stopPropagation(); quickToggleTask('${task.id}')">
    ...
    upcomingTasksList.innerHTML = html;
}

function quickToggleTask(taskId) {
